import { Dialog } from 'primereact/dialog';
import { useTranslation } from 'react-i18next';
import React, { forwardRef, useImperativeHandle, useState } from 'react';
import Enumeration from '@lib/enum';
import { InputText } from 'primereact/inputtext';
import { Button } from 'primereact/button';
import classNames from 'classnames';
import CommonFunction from '@lib/common';
import { Dropdown } from 'primereact/dropdown';
import _ from "lodash";
import EmptyDataCompact from "@ngdox/ui-lib/dist/components/empty-data/EmptyDataCompact";
import { ReactSortable } from "react-sortablejs";
import { XLayout, XLayout_Center, XLayout_Title } from '@ui-lib/x-layout/XLayout';
import FieldEntityApi from 'services/config/FieldEntityApi';
import "./scss/CustomFieldSettingsDetail.scss";
import { Checkbox } from 'primereact/checkbox';
import { InputNumber } from 'primereact/inputnumber';

function CustomFieldSettingsDetail(props, ref) {
    const t = CommonFunction.t;
    const { checkPermission } = props;

    // default validate object
    let emptyCustomFieldValidate = {
        type: null,
        code: null,
        name: null,
        listItems: null,
        configId: null
    }

    const type = [
        { name: t("custom-field.data-type.string"), code: 'STRING' },
        { name: t("custom-field.data-type.long"), code: 'LONG' },
        { name: t("custom-field.data-type.double"), code: 'DOUBLE' },
        { name: t("custom-field.data-type.date"), code: 'DATE' },
        { name: t("custom-field.data-type.datetime"), code: 'DATE_TIME' },
        { name: t("custom-field.data-type.boolean"), code: 'BOOLEAN' },
        { name: t("custom-field.data-type.list-single"), code: 'LIST_SINGLE' },
        { name: t("custom-field.data-type.list-multi"), code: 'LIST_MULTI' },
    ];

    const method = [
        { name: t("custom-field.method.array"), code: 'METHOD_ARRAY' },
        { name: t("custom-field.method.config"), code: 'METHOD_CONFIG' },
    ]


    const [show, setShow] = useState(false);
    const [mode, setMode] = useState(Enumeration.crud.none);
    const [customField, setCustomField] = useState(null);
    const [customFieldValidate, setCustomFieldValidate] = useState(emptyCustomFieldValidate);
    const [customfieldConfigList, setCustomFieldConfigList] = useState([]);
    const defaultValidConfig = {
        // string
        minLength: null,
        maxLength: null,
        // number
        greaterThan: null,
        greaterThanOrEqualTo: null,
        lessThan: null,
        lessThanOrEqualTo: null,
        // all
        regex: undefined
    }
    const [validConfig, setValidConfig] = useState(defaultValidConfig);

    useImperativeHandle(ref, () => ({
        create: (_customInfo) => {
            create(_customInfo);
        },
        update: (application, entity, id) => {
            update(application, entity, id);
        },
        init: async (_mode, _workflowId, _customField) => {

            // setMode(_mode);

            // switch (_mode) {
            //     case Enumeration.crud.update:
            //         let data = await WorkflowApi.getCustomField(_customField.id);
            //         setCustomField({
            //             ...data,
            //             status: data.status === 1 ? true : false,
            //             type: type.find(_type => _type.code === data.type),
            //             listMethod: method.find(_method => _method.code === data.listMethod),
            //             workflowId: _workflowId
            //         });
            //         break;
            //     default:
            //         setCustomField({ ...emptyCustomField, workflowId: _workflowId });
            //         break;
            // }

            // setCustomFieldValidate(emptyCustomFieldValidate);

            // getCustomFieldConfig();

            // setShow(true);
        }
    }));

    /**
     * create
     */
    const create = (_customInfo) => {
        let _customField = {
            application: null,
            entity: null,
            entityId: null,
            type: "",
            code: "",
            name: "",
            description: "",
            customize: null,
            listMethod: null,
            listItems: null,
            status: 1,
            configId: null
        }

        if (_customInfo) {
            _customField = { ..._customField, ..._customInfo };
        }

        setValidConfig({ ...defaultValidConfig });

        setMode(Enumeration.crud.create);

        setCustomField(_customField);

        setCustomFieldValidate(emptyCustomFieldValidate);

        getCustomFieldConfig();

        setShow(true);
    }

    /**
     * create
     */
    const update = (id) => {
        FieldEntityApi.get(id).then(res => {
            if (res) {
                setCustomField({
                    ...res,
                    type: type.find(_type => _type.code === res.type),
                    listMethod: method.find(_method => _method.code === res.listMethod)
                });

                setValidConfig(Object.assign(defaultValidConfig, res.validConfig));

                setMode(Enumeration.crud.update);

                setCustomFieldValidate(emptyCustomFieldValidate);

                getCustomFieldConfig();

                setShow(true);
            }
        })

    }


    /**
     * get custom field configs
     */
    const getCustomFieldConfig = () => {
        // get custom field config
        FieldEntityApi.getFieldDataConfig().then(res => {
            if (res) {
                res.forEach(el => {
                    el.display = el.code ? `${el.code} - ${el.name}` : el.name;
                });
                setCustomFieldConfigList(res);
            }
        })
    }

    /**
     * hide detail
     */
    const hideDetail = () => {
        setShow(false);
    };

    /**
     * submit
     */
    const submit = async () => {
        if (checkPermission && typeof checkPermission === "function") {
            let valid = checkPermission('update')
            if (!valid) {
                CommonFunction.toastError(t("common.you-do-not-have-permission"));
                return;
            }
        }
        // validate
        let isValid = performValidate();

        // valid custom field list item
        if (isValid) {
            if (customField.listMethod && customField.listMethod.code == 'METHOD_ARRAY') {
                if (!customField.listItems || customField.listItems.length === 0) {
                    isValid = false;
                    CommonFunction.toastWarning(t("workflow.custom-field.empty-value-list"));
                }
            }
        }

        // submit
        if (isValid) {
            // call api
            try {
                let _customField = {
                    ...customField,
                    entityId: (props.entityId || 0),
                    type: customField.type.code,
                    listMethod: customField.listMethod ? customField.listMethod.code : customField.listMethod,
                    configId: customField.configId,
                    validConfig: { ...validConfig }
                };
                let result = null;

                switch (mode) {
                    case Enumeration.crud.create:
                        result = await FieldEntityApi.create(_customField);
                        break;
                    case Enumeration.crud.update:
                        result = await FieldEntityApi.update(_customField);
                        break;
                    default:
                        break;
                }
                if (result) {
                    props["after-submit"](mode, result);
                    hideDetail();
                    CommonFunction.toastSuccess(t("common.save-success"));
                }
            } catch (error) {
                console.log(error);
            }
        }
    };

    /**
    * validate service
    * @param {Array} _props [] = validate all, ['a','b'] = validate prop a & b
    */
    const performValidate = (_customField, _props) => {
        _customField = _customField || _.cloneDeep(customField);
        _props = _props || [];
        let result = { ...customFieldValidate }, isValid = true;

        // validate all props
        if (_props.length === 0) {
            for (const property in result) {
                _props.push(property);
            }
        }

        // validate props
        _props.forEach(prop => {
            switch (prop) {
                case 'type':
                    result[prop] = _customField.type ? null : `${t('custom-field.type')} ${t('message.cant-be-empty')}`;
                    break;
                case 'code':
                    if (_customField.code.length > 0) {
                        let currentId = mode === Enumeration.crud.create ? -1 : _customField.id;
                        result[prop] = CommonFunction.isExist(props.customFields.filter(o => o.id !== currentId), 'code', _customField) ? `${t('custom-field.code')} ${t('message.has-exist')}` : null;
                    } else {
                        result[prop] = _customField.code.length > 0 ? null : `${t('custom-field.code')} ${t('message.cant-be-empty')}`;
                    }
                    break;
                case 'name':
                    result[prop] = _customField.name.length > 0 ? null : `${t('custom-field.name')} ${t('message.cant-be-empty')}`;
                    break;
                case 'configId':
                    if (_customField.listMethod && _customField.listMethod.code === "METHOD_CONFIG") {
                        result[prop] = _customField.configId ? null : t('validate.required');
                    }
                    break;
                case 'listItems':
                    if (!_customField.listItems) break;
                    const listItems = _.cloneDeep(_customField.listItems);
                    const listNews = listItems.filter(o => o.isNew);
                    const listOlds = _.difference(listItems, listNews);
                    if (listNews && listNews.length > 0) {
                        listNews.forEach(item => {
                            if (CommonFunction.isExist(listOlds, 'value', item)) {
                                result[prop] = `${t('field.value')} ${t('message.is-duplicate')}`;
                            } else {
                                result[prop] = null;
                            }
                        });
                    } else {
                        result[prop] = null;
                    }
                    break;
                default:
                    break;
            }
        });

        // set state
        setCustomFieldValidate(result);

        // check if object has error
        for (const property in result) {
            if (result[property]) {
                isValid = false;
                break;
            }
        }

        return isValid;
    };

    /**
     * apply creating/editing service prop on input change
     * @param {string} prop
     * @param {*} val
     */
    const applyChange = (prop, val) => {
        let _customField = _.cloneDeep(customField);
        let needValidate = [prop];
        // set state value
        switch (prop) {
            case "type":
                if (val.code !== 'LIST_SINGLE' || val.code !== 'LIST_MULTI') _customField.listMethod = null;
                break;
            case "name":
                if (mode === "create") {
                    _customField.code = generateKey(val);
                    needValidate = ["name", "code"]
                }
                break;
            default:
                break;
        }
        _customField[prop] = val;
        performValidate(_customField, needValidate);
        setCustomField(_customField);
    };

    /**
     * add value list
     * @param {*} index
     */
    const addValueList = (index) => {
        index = index === null ? 0 : index;
        let _customField = _.cloneDeep(customField);
        if (CommonFunction.isEmpty(_customField.listItems)) {
            _customField.listItems = []
        }

        _customField.listItems = [
            ..._customField.listItems.slice(0, index + 1),
            {
                key: "",
                value: ""
            },
            ..._customField.listItems.slice(index + 1)
        ]

        setCustomField(_customField);
    }

    /**
     * add value list
     * @param {*} index
     */
    const removeValueList = (index) => {
        let _customField = _.cloneDeep(customField);
        _customField.listItems = [
            ..._customField.listItems.slice(0, index),
            ..._customField.listItems.slice(index + 1)
        ];
        setCustomField(_customField);
    }

    /**
     * set value list
     * @param {*} index
     */
    const setValueList = (index, prop, value) => {
        let _customField = _.cloneDeep(customField);

        // prepare key
        if (prop === "value") {
            // check if old key is auto generated key
            let oldGeneratedKey = generateKey(_customField.listItems[index].value);
            let oldKey = _customField.listItems[index].key;
            if (oldGeneratedKey === oldKey) {
                // if old key is auto generated key, set new key
                _customField.listItems[index].key = generateKey(value);
            }
        } else {
            // if change key, standalize key
            value = generateKey(value);
        }

        // apply change
        _customField.listItems[index][prop] = value;

        setCustomField(_customField);
    }

    /**
     * generate key
     * remove vietnamese charater
     * remove double space
     * replace space to _
     * @param {*} value
     * @returns
     */
    const generateKey = (value) => {
        return CommonFunction.removeAccentVietnamese(value).replace(/\s\s+/g, ' ').trim().replaceAll(" ", "_");
    }

    /**
     * set actions event after sort
     */
    const afterSortEvents = (e, sortable, store) => {
        let oldIndex = e.oldIndex, newIndex = e.newIndex;
        if (oldIndex !== undefined && newIndex !== undefined && oldIndex !== newIndex) {
            let _customField = _.cloneDeep(customField);

            // move item in array
            CommonFunction.arrayMove(_customField.listItems, oldIndex, newIndex);

            // set state
            setCustomField(_customField);
        }
    }

    /**
     * apply valid config change
     * @param {*} prop 
     * @param {*} value 
     */
    const applyValidConfigChange = (prop, value) => {
        let _validConfig = _.cloneDeep(validConfig);
        _validConfig[prop] = value;
        setValidConfig(_validConfig);
    }

    /**
     * render validate config
     */
    const renderValidConfig = () => {

        if (customField.type && customField.type.code) {
            switch (customField.type.code) {
                case "STRING":
                    return (
                        <div className='grid formgrid p-fluid fluid '>
                            <div className='col-12'>
                                <XLayout_Title>{t("customfield.validate-title")}</XLayout_Title>
                            </div>
                            <div className='col-6'>
                                <span className="p-float-label">
                                    <InputNumber
                                        value={validConfig.minLength}
                                        onChange={(e) => applyValidConfigChange("minLength", e.value)}
                                        min={0}
                                    ></InputNumber>
                                    <label>{t("customfield.valid-config.minLength")}</label>
                                </span>
                            </div>
                            <div className='col-6'>
                                <span className="p-float-label">
                                    <InputNumber
                                        value={validConfig.maxLength}
                                        onChange={(e) => applyValidConfigChange("maxLength", e.value)}
                                        min={0}
                                    ></InputNumber>
                                    <label>{t("customfield.valid-config.maxLength")}</label>
                                </span>
                            </div>
                            <div className='col-12'>
                                <span className="p-float-label">
                                    <InputText
                                        value={validConfig.regex}
                                        onChange={(e) => applyValidConfigChange("regex", e.target.value)}
                                    ></InputText>
                                    <label>{t("customfield.valid-config.regex")}</label>
                                </span>
                            </div>
                        </div>
                    )
                case "LONG":
                case "DOUBLE":
                    let customNumberConfig = {};
                    if (customField.type.code === "DOUBLE") {
                        customNumberConfig = {
                            mode: "decimal",
                            minFractionDigits: 1,
                            maxFractionDigits: 20
                        }
                    }

                    return (
                        <div className='grid formgrid p-fluid fluid '>
                            <div className='col-12'>
                                <XLayout_Title>{t("customfield.validate-title")}</XLayout_Title>
                            </div>
                            <div className='col-6'>
                                <span className="p-float-label">
                                    <InputNumber
                                        value={validConfig.greaterThan}
                                        onChange={(e) => applyValidConfigChange("greaterThan", e.value)}
                                        {...customNumberConfig}
                                    ></InputNumber>
                                    <label>{t("customfield.valid-config.greaterThan")}</label>
                                </span>
                            </div>
                            <div className='col-6'>
                                <span className="p-float-label">
                                    <InputNumber
                                        value={validConfig.lessThan}
                                        onChange={(e) => applyValidConfigChange("lessThan", e.value)}
                                        {...customNumberConfig}
                                    ></InputNumber>
                                    <label>{t("customfield.valid-config.lessThan")}</label>
                                </span>
                            </div>
                            <div className='col-6'>
                                <span className="p-float-label">
                                    <InputNumber
                                        value={validConfig.greaterThanOrEqualTo}
                                        onChange={(e) => applyValidConfigChange("greaterThanOrEqualTo", e.value)}
                                        {...customNumberConfig}
                                    ></InputNumber>
                                    <label>{t("customfield.valid-config.greaterThanOrEqualTo")}</label>
                                </span>
                            </div>
                            <div className='col-6'>
                                <span className="p-float-label">
                                    <InputNumber
                                        value={validConfig.lessThanOrEqualTo}
                                        onChange={(e) => applyValidConfigChange("lessThanOrEqualTo", e.value)}
                                        {...customNumberConfig}
                                    ></InputNumber>
                                    <label>{t("customfield.valid-config.lessThanOrEqualTo")}</label>
                                </span>
                            </div>
                            <div className='col-12'>
                                <span className="p-float-label">
                                    <InputText
                                        value={validConfig.regex}
                                        onChange={(e) => applyValidConfigChange("regex", e.target.value)}
                                    ></InputText>
                                    <label>{t("customfield.valid-config.regex")}</label>
                                </span>
                            </div>
                        </div>
                    )
                default:
                    break;
            }
        }
        return <></>
    }

    if (!customField) {
        return <></>
    }

    return (
        <Dialog
            header={t(`common.dialog.title.${mode === Enumeration.crud.create ? "create" : "update"}`).format(t("workflow.custom-field"))}
            visible={show}
            modal
            className="wd-800-600"
            contentClassName="p-0"
            onHide={hideDetail}
            footer={
                <>
                    <Button label={t('common.cancel')} icon="bx bx-x" className="p-button-text text-muted" onClick={hideDetail} />
                    <Button label={t('common.save')} icon="bx bxs-save" className="primary" onClick={submit} />
                </>
            }
        >
            <XLayout className="px-2 pb-2" >
                <XLayout_Center className="p-0">
                    <div className="p-fluid fluid  formgrid grid p-0">
                        <div className="col-6">
                            <span className="p-float-label">
                                <InputText id="name" value={customField.name}
                                    onChange={(e) => applyChange('name', e.target.value)}
                                    className={classNames({ 'p-invalid': customFieldValidate.name })}
                                    disabled={customField.customize === false}
                                />
                                <label htmlFor="name" className="require">{t('custom-field.name')}</label>
                            </span>
                            {customFieldValidate.name && <small className="p-invalid">{customFieldValidate.name}</small>}
                        </div>
                        <div className="col-6">
                            <span className="p-float-label">
                                <InputText id="code"
                                    value={customField.code}
                                    onChange={(e) => applyChange('code', e.target.value)}
                                    className={classNames({ 'p-invalid': customFieldValidate.code })}
                                    disabled={customField.customize === false}
                                />
                                <label htmlFor="code" className="require">{t('custom-field.code')}</label>
                            </span>
                            {customFieldValidate.code && <small className="p-invalid">{customFieldValidate.code}</small>}
                        </div>

                        <div className="col-12">
                            <span className="p-float-label">
                                <InputText id="description" value={customField.description}
                                    
                                    onChange={(e) => applyChange('description', e.target.value)} />
                                <label htmlFor="description">{t('custom-field.description')}</label>
                            </span>
                        </div>

                        {customField.customize === false &&
                            <div className="col-12">
                                <div className="p-field-radiobutton mb-0">
                                    <Checkbox
                                        inputId="transfer"
                                        name="transfer"
                                        onChange={(e) => applyChange('transfer', e.checked)}
                                        checked={customField.transfer}
                                    />
                                    <label htmlFor="transfer">{t('custom-field.transfer')}</label>
                                </div>
                            </div>
                        }

                        <div className="col-12">
                            <span className="p-float-label">
                                <Dropdown value={customField.type} options={type} optionLabel="name"
                                    onChange={(e) => applyChange('type', e.value)}
                                    className={classNames({ 'p-invalid': customFieldValidate.type })}
                                    disabled={customField.customize !== false ? (customField.id ? true : false) : false}
                                />
                                <label className="require">{t('custom-field.type')}</label>
                            </span>
                            {customFieldValidate.type && <small className="p-invalid">{customFieldValidate.type}</small>}
                        </div>

                        {(customField.type.code == 'LIST_SINGLE' || customField.type.code == 'LIST_MULTI') && (
                            <div className="col-12">
                                <span className="p-float-label">
                                    <Dropdown value={customField.listMethod} options={method} optionLabel="name"
                                        onChange={(e) => applyChange('listMethod', e.value)}
                                        className={classNames({ "dense": true, 'p-invalid': customFieldValidate.listMethod })}
                                    />
                                    <label htmlFor="name">{t('custom-field.method')}</label>
                                </span>
                                {customFieldValidate.listMethod && <small className="p-invalid">{customFieldValidate.listMethod}</small>}
                            </div>
                        )}

                        {(customField.listMethod && customField.listMethod.code == 'METHOD_CONFIG') && (
                            <div className="col-12">
                                <span className="p-float-label">
                                    <Dropdown
                                        value={customField.configId}
                                        options={customfieldConfigList}
                                        optionLabel="display"
                                        optionValue="id"
                                        onChange={(e) => applyChange('configId', e.value)}
                                        className={classNames({ "dense": true, 'p-invalid': customFieldValidate.configId })}
                                        filter
                                    />
                                    <label htmlFor="name">{t('custom-field.data-config')}</label>
                                    {customFieldValidate.configId && <small className="p-invalid">{customFieldValidate.configId}</small>}
                                </span>
                            </div>
                        )}

                        {(customField.listMethod && customField.listMethod.code == 'METHOD_ARRAY') &&
                            <>
                                {(!customField.listItems || customField.listItems.length === 0) &&
                                    <EmptyDataCompact message={t("workflow.custom-field.empty-value-list")}>
                                        <Button
                                            className="ml-2 width-fit-content primary"
                                            label={t("workflow.custom-field.add-value-list")}
                                            icon="bx bx-plus"
                                            onClick={addValueList}
                                        ></Button>
                                    </EmptyDataCompact>
                                }

                                {customField.listItems && customField.listItems.length > 0 &&
                                    <div className="custom-field-detail-choose-list">
                                        <div className="list-row">
                                            <div className="row-action"></div>
                                            <div className="bold-and-color">{t("workflow.custom-field.value-list-value")}</div>
                                            <div className="bold-and-color">{t("workflow.custom-field.value-list-key")}</div>
                                        </div>
                                        <ReactSortable
                                            animation="500"
                                            list={customField.listItems}
                                            setList={() => { }}
                                            onEnd={(evt, sortable, store) => {
                                                afterSortEvents(evt, sortable, store);
                                            }}>
                                            {customField.listItems.map((listItem, index) => (
                                                <div key={index} className="list-row">
                                                    <div className="row-action">
                                                        <Button icon="bx bx-menu text-grey-7" className="p-button-text p-button-rounded" tooltip={t("common.point-and-order")}></Button>
                                                        <Button icon="bx bx-plus text-green-9" className="p-button-text p-button-rounded" onClick={() => addValueList(index)}></Button>
                                                        <Button icon="bx bx-x text-red-9" className="p-button-text p-button-rounded" onClick={() => removeValueList(index)}></Button>
                                                    </div>
                                                    <div style={{ paddingRight: "0.125rem" }}>
                                                        <InputText
                                                            

                                                            value={listItem.value}
                                                            onChange={(e) => setValueList(index, "value", e.target.value)} />
                                                    </div>
                                                    <div style={{ paddingLeft: "0.125rem" }}>
                                                        <InputText
                                                            
                                                            value={listItem.key}
                                                            onChange={(e) => setValueList(index, "key", e.target.value)} />
                                                    </div>
                                                </div>

                                            ))}
                                        </ReactSortable>
                                    </div>
                                }
                            </>
                        }

                    </div>
                    {renderValidConfig()}
                </XLayout_Center>
            </XLayout >
        </Dialog >

    )
}

CustomFieldSettingsDetail = forwardRef(CustomFieldSettingsDetail);

export default CustomFieldSettingsDetail;


