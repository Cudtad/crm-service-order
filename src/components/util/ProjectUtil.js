...
const ProjectUtil = {
    ...
    isPM(state) {
        let _found = false;
        if (state.userRoles) {
            state.userRoles.forEach(r => {
                if (r.userId === state.user.id && r.roleName === "PM") {
                    _found = true;
                }
            })
        }
    ...
    endDateValidate(obj) {
        if (obj.endDate == null) {
            return `${i18n.t('common.enddate')} ${i18n.t('message.cant-be-empty')}`;
        } else if (obj.startDate &&  obj.endDate && new Date(obj.endDate) < new Date(obj.startDate)) {
            return `${i18n.t('common.enddate')} ${i18n.t('message.cant-be-before-start-date')}`;
        } else {
            return null;
        }
    },
    ...
    startDateValidate(obj) {
        if (obj.startDate == null) {
            return `${i18n.t('common.startdate')} ${i18n.t('message.cant-be-empty')}`;
        } else if (obj.startDate && obj.endDate && new Date(obj.startDate) > new Date(obj.endDate)) {
            return `${i18n.t('common.startdate')} ${i18n.t('message.cant-be-after-end-date')}`;
        } else {
            return null;
        }
    },
    ...
    dateValidate(child, parent, prop) {
        ...
        switch (prop) {
            case 'startDate':
                if (!child[prop]) {
                    message = `${i18n.t('common.start-date')} ${i18n.t('message.cant-be-empty')}`;
                } else if (new Date(child.endDate) && new Date(child[prop]) > new Date(child.endDate)) {
                    message = `${i18n.t('common.start-date')} ${i18n.t('message.cant-be-after-end-date')}`;
                } else if (new Date(child[prop]) < new Date(parent[prop])) {
                    message = `${i18n.t('common.start-date')} ${i18n.t('message.date.must-be-after-or-equal')} ${CommonFunction.formatDate(parent.startDate, 'DD/MM/YYYY')}`;
                } else {
                    message = null;
                }
                break;
            case 'endDate':
                if (!child[prop]) {
                    message = `${i18n.t('common.end-date')} ${i18n.t('message.cant-be-empty')}`;
                } else if (new Date(child.startDate) && new Date(child[prop]) < new Date(child.startDate)) {
                    message = `${i18n.t('common.end-date')} ${i18n.t('message.cant-be-before-start-date')}`;
                } else if (new Date(child[prop]) > new Date(parent[prop])) {
                    message = `${i18n.t('common.end-date')} ${i18n.t('message.date.must-be-before-or-equal')} ${CommonFunction.formatDate(parent.endDate, 'DD/MM/YYYY')}`;
                } else {
                    message = null;
                }
                break;
